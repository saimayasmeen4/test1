...
from dataset2 import prepare_data, Dataset
from utils import *
...
parser.add_argument("--train_images", type=int, default=10, help='total images to train, should be divisible of 2 and less than 18000')
parser.add_argument("--val_images", type=int, default=10, help='total images to validate, should be divisible of 2 and less than 2k')
...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            # pass noisy image as input
            imgn_train = data[: , 0] 
            # use clean image as label
            img_train = data[: , 1]

            # training step
            model.train()
        
            img_train, imgn_train = Variable(img_train.to(device)), Variable(imgn_train.to(device)) #tcw201809131425tcw
            ...
            optimizer.step()
			      ...
			      # comment below 2 lines to save time and uncomment 3rd line
            # model.eval()
        ...
        for k in range(len(dataset_val)):
            ...
            img_val, imgn_val = Variable(img_val.to(device)), Variable(imgn_val.to(device),requires_grad=False)
...
if __name__ == "__main__":
    if opt.preprocess:
      prepare_data(data_path='data', patch_size=41, stride=10, aug_times=4, train_images=opt.train_images, val_images=opt.val_images) #tcw201810102244
    main()
